...
if (isset($_REQUEST['update_id'])) {
    try {
        ...
        $select_stmt = $db->prepare('SELECT * FROM sp14 WHERE id = :id');
...
if (isset($_REQUEST['btn_update'])) {
    try {
        ...
        if (!isset($errorMsg)) {
            ...
            $update_stmt = $db->prepare("UPDATE sp14 SET name014 =:name014 ,studentid014 =:studentid014 ,branch014 =:branch014 ,faculty014 =:faculty014 ,name114 =:name114
            ,name214 =:name214 ,rank214 =:rank214 ,department214 =:department214 ,name314 =:name314  ,name414 =:name414   
            ...
            $update_stmt->bindParam(':name014', $name014);
            $update_stmt->bindParam(':studentid014', $studentid014);
            $update_stmt->bindParam(':branch014', $branch014);
            $update_stmt->bindParam(':faculty014', $faculty014);
            $update_stmt->bindParam(':name114', $name114);
            $update_stmt->bindParam(':name214', $name214);
            $update_stmt->bindParam(':rank214', $rank214);
            $update_stmt->bindParam(':department214', $department214);
            $update_stmt->bindParam(':name314', $name314);
            $update_stmt->bindParam(':name414', $name414);
...
<section class="bg-light w-100">
    <div class="container py-5">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="name014" value="<?php echo $row['name014']; ?>" >
        </div>
        <div class="mb-3">
            <label for="fullname" class="form-label">รหัสประจำตัวนักศึกษา</label>
            <input type="text" class="form-control" name="studentid014" value="<?php echo $row['studentid014']; ?>" >
        </div>
        <div class="mb-3">
            <label for="fullname" class="form-label">สาขาวิชา</label>
            <input type="text" class="form-control" name="branch014" value="<?php echo $row['branch014']; ?>" >
        </div>
        <div class="mb-3">
            <label for="fullname" class="form-label">คณะ / วิทยาลัย </label>
            <input type="text" class="form-control" name="faculty014" value="<?php echo $row['faculty014']; ?>" >
        </div>
        <div class="mb-3">
            <label for="fullname" class="form-label">ชื่อสถานประกอบการ</label>
            <input type="text" class="form-control" name="name114" value="<?php echo $row['name114']; ?>" >
        </div>
        <div class="mb-3">
            <label for="fullname" class="form-label">ชื่อ-นามสกุลผู้ประเมิน</label>
            <input type="text" class="form-control" name="name214" value="<?php echo $row['name214']; ?>" >
        </div>
        <div class="mb-3">
            <label for="fullname" class="form-label">ตำแหน่ง</label>
            <input type="text" class="form-control" name="rank214" value="<?php echo $row['rank214']; ?>" >
        </div>
        <div class="mb-3">
            <label for="fullname" class="form-label">แผนก/ฝ่าย </label>
            <input type="text" class="form-control" name="department214" value="<?php echo $row['department214']; ?>" >
        </div>
        <h2>หัวข้อรายงาน /Report title </h2>
        <div class="mb-3">
            <label for="fullname" class="form-label">ภาษาไทย / Thai</label>
            <input type="text" class="form-control" name="name314" value="<?php echo $row['name314']; ?>" >
        </div>
        <div class="mb-3">
            <label for="fullname" class="form-label">ภาษาอังกฤษ / English</label>
            <input type="text" class="form-control" name="name414" value="<?php echo $row['name414']; ?>" >
        </div>
        <hr> 
