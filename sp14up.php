...
require_once('connection.php');
    ...
    if (isset($_SESSION['user_login'])) {
        ...
        $stmt1 = $db->query("SELECT * FROM sp14a WHERE user_id = $user_id");
        $stmt1->execute();
    ...
    if (isset($_REQUEST['btn_update'])) {
        try {
            ...
            $path = "upload/sp14/".$image_file1;
            $directory = "upload/sp14/"; // set uplaod folder path for upadte time previos file remove and new file upload for next use
                ...
                if ($type1 == "image/jpg" || $type1 == 'image/jpeg' || $type1 == "image/png" || $type1 == "image/gif") {
                    if (!file_exists($path)) { // check file not exist in your upload folder path
                        if ($size1 < 5000000) { // check file size 5MB
                            ...
                            move_uploaded_file($temp1, 'upload/sp14/'.$image_file1); // move upload file temperory directory to your upload folder
                            move_uploaded_file($temp2, 'upload/sp14/'.$image_file2); // move upload file temperory directory to your upload folder
                        } else {
            ...
            if (!isset($errorMsg)) {
                $insert_stmt = $db->prepare('INSERT INTO sp14a(image1,image2,user_id) VALUES (:file_up1,:file_up2,:id)');
