...
if( $_GET['disid'])
{
	...
	$query=mysqli_query($con,"update tblcomments set status='2' where id='$id'");
...
if($_GET['appid'])
{
    ...
    $query2=mysqli_query($con,"update tblposts set limitcount = limitcount-'1' where id='$postid'");
	$msg="ย้อนกลับ";
}
...
<td>
<?php if($st==0):?>
    <a href="manage-comments.php?disid=<?php echo htmlentities($row['id']);?> &&hourspost=<?php echo htmlentities($row['hourspost']);?> &&user_id=<?php echo htmlentities($row['user_id']);?> &&hoursactivity=<?php echo htmlentities($row['hoursactivity']);?>" title="Disapprove this comment"><i class="ion-arrow-return-right" style="color: #29b6f6;"></i></a> 
<?php else :?>
...
<?php endif;?>
 ...
 <td>
  <a href="manage-comments.php?appid=<?php echo htmlentities($row['id']);?>&&pid=<?php echo htmlentities($row['postid']);?>" title="Approve this comment"><i class="ion-arrow-return-right" style="color: #29b6f6;"></i></a> 
 </td>
