...
require_once('connection.php');
    ...
    if (isset($_SESSION['user_login'])) {
        ...
        $stmt1 = $db->query("SELECT * FROM sp13a WHERE user_id = $user_id");
        $stmt1->execute();
    ...
    if (isset($_REQUEST['btn_update'])) {
        try {
            ...
            $path = "upload/sp13/".$image_file1;
            $directory = "upload/sp13/"; // set uplaod folder path for upadte time previos file remove and new file upload for next use
                ...
                if ($type1 == "image/jpg" || $type1 == 'image/jpeg' || $type1 == "image/png" || $type1 == "image/gif") {
                    if (!file_exists($path)) { // check file not exist in your upload folder path
                        if ($size1 < 5000000) { // check file size 5MB
                            ...
                            move_uploaded_file($temp1, 'upload/sp13/'.$image_file1); // move upload file temperory directory to your upload folder
                            move_uploaded_file($temp2, 'upload/sp13/'.$image_file2); // move upload file temperory directory to your upload folder
                        } else {
            ...
            if (!isset($errorMsg)) {
                $insert_stmt = $db->prepare('INSERT INTO sp13a(image1,image2,user_id) VALUES (:file_up1,:file_up2,:id)');
                $insert_stmt->bindParam(':file_up1', $image_file1);
                $insert_stmt->bindParam(':file_up2', $image_file2);
                $insert_stmt->bindParam(':id', $user_id);

                if ($insert_stmt->execute()) {
                    $updateMsg = "ไฟล์ อัพเดทเรียบร้อยแล้ว...";
                    header("refresh:2;visituser.php");
                }
            }
 ...
 <nav id="main_nav" class="navbar navbar-expand-lg navbar-light bg-white shadow">
    ...
    <div class="container text-center">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <input type="file" name="txt_file1" class="form-control" value="<?php echo $row1['image1']; ?>">
            </div>
            </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <input type="file" name="txt_file2" class="form-control" value="<?php echo $row1['image2']; ?>">
            </div>
            </div>
        ...
        <div class="form-group">
            <div class="col-sm-12">
                ...
                <a href="visituser.php" class="btn btn-danger">Cancel</a>
            </div>
        </div>
    </form>
    </div>
